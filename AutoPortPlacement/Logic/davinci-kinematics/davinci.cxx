...
Eigen::Matrix3d zRotation(double theta)
{
  Eigen::Matrix3d R;
  double c = cos(theta);
  double s = sin(theta);
                                           ...
                                           const std::vector<double>& q) const
{
  ...
  T.topLeftCorner<3,3>() = T.topLeftCorner<3,3>()*zRotation(q[0]);
  ...
  T_1_2(3,3) = 1.0;
  T_1_2.topLeftCorner<3,3>() = T_1_2.topLeftCorner<3,3>()*zRotation(q[1]);
  T = T*T_1_2;
  ...
  T_2_3(3,3) = 1.0;
  T_2_3.topLeftCorner<3,3>() = T_2_3.topLeftCorner<3,3>()*zRotation(q[2]);
  T = T*T_2_3;
  ...
  T_4_5(3,3) = 1.0;
  T_4_5.topLeftCorner<3,3>() = T_4_5.topLeftCorner<3,3>()*zRotation(q[4]);
  T = T*T_4_5;
  ...
  T_5_6(3,3) = 1.0;
  T_5_6.topLeftCorner<3,3>() = T_5_6.topLeftCorner<3,3>()*zRotation(q[5]);
  T = T*T_5_6;

  // For gripper. Bring it in later
                                            ...
                                            std::vector<Collisions::Cylisphere>* cylispheres) const
{
  ...
  p(2) = -params_.eL_*s2a;
  p(3) = 1.0;
  c_h.p1 = (portFrame*p).head<3>();
  
  p(2) -= params_.el2_;
  c_h.p2 = (portFrame*p).head<3>();
                                  ...
                                  std::vector<Eigen::Matrix4d>* jointPoses) const
{
  ...
  T_3_4.topLeftCorner<3,3>() = T_3_4.topLeftCorner<3,3>()*zRotation(q[3]);
  T = T*T_3_4;
  jointPoses->push_back(T);
                                             ...
                                             Collisions::Sphere* sphere) const
{
  ...
  c.p1 = c.p2;
  c.p2 = c.p1 + poses[1].block<3,1>(0,1)*params_.pl2_;
  cylispheres->push_back(c);
  ...
  c.p2 = c.p1 + poses[2].block<3,1>(0,1)*params_.pl3_;
  cylispheres->push_back(c);
  ...
  sphere->p = poses[5].topRightCorner<3,1>() + poses[5].block<3,1>(0,2)*params_.eL_;
}
